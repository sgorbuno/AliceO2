                          ...
                          const char* outFileName = "TPCFastTransform_VoxRes.root", bool useSmoothed = false, bool invertSigns = false)
{
  ...
  */
  ...
  TNtuple* ntAll = new TNtuple("all", "all", "sec:row:x:y:z:cx:cy:cz:ix:iy:iz:cxC:cyC:czC:ixC:iyC:izC");
  ...
  debugFile->cd();
  TNtuple* ntInvAll = new TNtuple("invall", "invall", "sec:row:x:y:z:cx:cy:cz:cxC:cyC:czC");
  ...
  debugFile->cd();
  TNtuple* ntVox =
    new TNtuple("vox", "vox", "sec:row:n:x:y:z:vx:vy:vz:cx:cy:cz:ix:iy:iz:cxC:cyC:czC:ixC:iyC:izC");
  ...
  debugFile->cd();
  TNtuple* ntInvVox =
    new TNtuple("invvox", "invvox", "sec:row:n:x:y:z:vx:vy:vz:cx:cy:cz:cxC:cyC:czC");
  ...
  for (int direction = 0; direction < 2; direction++) { // 0 - normal, 1 - inverse
    ...
    for (int32_t iVox = 0; iVox < currentTree->GetEntriesFast(); iVox++) {
      ...
      float cxC, cyC, czC, ixC, iyC, izC;
      if (direction == 0) {
        getAllCorrections(iSector, iRow, y, z, cx, cy, cz, ix, iy, iz);
        getAllCorrections(iSector + geo.getNumberOfSectorsA(), iRow, y, -z, cxC, cyC, czC, ixC, iyC, izC);
        ...
        getInvCorrections(iSector + geo.getNumberOfSectorsA(), iRow, y, -z, cxC, cyC, czC);
        float ntEntry[] = {(float)iSector, (float)iRow, voxEntries,
                           (float)x, (float)y, (float)z,
                           (float)correctionX, (float)correctionY, (float)correctionZ,
                           (float)cx, (float)cy, (float)cz,
    ...
    for (int32_t iSector = 0; iSector < geo.getNumberOfSectorsA(); iSector++) {
      // for (int32_t iSector = 0; iSector < 1; iSector++) {
