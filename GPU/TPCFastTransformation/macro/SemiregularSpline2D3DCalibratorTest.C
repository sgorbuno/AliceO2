...
root -l SemiregularSpline2D3DCalibratorTest.C++
...
#include "GPU/SemiregularSpline2D3DCalibrator.h"
...
bool initTPC(const char* fileName, int slice, int row) //not in use
{
  // open NTuple file
...
int SemiregularSpline2D3DCalibratorTest()
{
  ...
  SemiregularSpline2D3DCalibrator finder;

  TCanvas* canv = nullptr;
  if (kDraw) {
    canv = new TCanvas("cQA", "2D spline calibration", 900, 500);
    canv->Divide(2, 2);
  }
    ...
    */
    ...
    do {
      for (int i=0; i< finder.getSpline().getGridV().getNumberOfKnots() ; i++){
        cout <<  "knots u in row " << i << ":" << finder.getSpline().getGridU(i).getNumberOfKnots()
      ...
      const RegularSpline1D& gridV = finder.getSpline().getGridV();

      int nKnots = 0;
      for (int i = 0; i < gridV.getNumberOfKnots(); i++) {
	double v = gridV.knotIndexToU( i );
	const RegularSpline1D& gridU = finder.getSpline().getGridU(i);	
    ...
    //finder.getSpline().getGridV().Print();
